    ...
    if (txIds.length > 0) {
      const [itemsRes, pricingRes, buyersRes, participantsRes] = await Promise.all([
        ...
        adminClient
          ...
          .select("transaction_id, item_amount, buyer_total_amount, seller_net_amount, platform_fee_amount, processing_fee_amount, currency_code")
          .in("transaction_id", txIds),
        buyerIds.length > 0
          ? adminClient
              .from("profiles")
    ...
    if (completedIds.length > 0) {
      ...
      if (completedPricing) {
        for (const p of completedPricing) {
          summary.total_earned += (p.seller_net_amount as number) ?? 0;
        }
      }
    }
