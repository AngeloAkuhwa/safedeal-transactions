...
ALTER TABLE public.permission_change_sets
  ...
  CHECK (target_scope = ANY (ARRAY['role','user','template','permission','orchestration_rules']));
...
BEGIN
  ...
  ELSIF cs.target_scope = 'orchestration_rules' THEN
    ...
    INSERT INTO public.assignment_rule_versions(rule_id, version, config, actor_id, note)
    VALUES (rule_row.id, next_ver, after_json, auth.uid(),
