...
-- 9) Idempotency enforcement trigger for ledger writes
...
BEGIN
  ...
  LOOP
    ...
    IF v_new ~ 'INSERT INTO public\.escrow_ledger_entries\([^;]*?''dispute_unfreeze''' THEN
...
-- 12) Remaining dispute ledger entries also go through the guarded writer.
DO $do$
DECLARE
  r record;
  v_src text;
