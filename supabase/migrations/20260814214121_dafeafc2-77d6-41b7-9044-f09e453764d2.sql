-- Second live blocker on the same rail: validate_money_transition does NOT
-- allow funds_held_in_escrow -> refund_pending. Refunding money that is still
-- held therefore aborted on the transitions trigger. The legal route is to
-- freeze first, which is also the honest description of what happens: the
-- funds stop being releasable the moment a refund is committed.
...
BEGIN
  ...
  -- Walk the legal path to refund_pending, recording every hop.
...
-- Harness: seed the way the real payment flow does (not_secured -> payment_pending).
...
DECLARE
  ...
  v_tx2 uuid; v_payment2 uuid; v_money text;
BEGIN
  BEGIN
    ...
    v_checks := v_checks || jsonb_build_object(
      ...
      'pass', public.start_refund_atomic(v_tx, v_item + v_platform, v_buyer, 'selftest', 'repeat') = v_refund);
