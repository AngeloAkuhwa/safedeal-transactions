...
| 6.2 | Deliberate visual convergence: fold the stray shades the mechanical rule had to leave into their tones, in small per-tone PRs, each with a preview. Batch 1 (#75): all 127 yellow utilities across 21 admin files folded into amber, retiring yellow as an admin hue; the colour-law contract now bans it outright (red-verified), and the specimen preview shows each composite before and after on the real ground. Remaining: orange-300 accents, /20 washes, 500/600 solids, hover one-offs, and consuming the newly exact amber sets mechanically. | in progress (batches 1 to 3: #75 to #77; admin count 3411 to 3253; batch 3 settled standalone caution text at amber-300, previewed on the fold specimen) |
