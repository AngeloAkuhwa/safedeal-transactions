...
| 6.2 | Deliberate visual convergence: fold the stray shades the mechanical rule had to leave (yellow triads, orange-300 accents, /20 washes, 500/600 solids, hover one-offs) into their tones. This moves pixels, so it ships in small per-tone PRs, each with a preview. | pending, after 6.1 |
...
| 6.4 | Raw `<img>` migration: 65 sites across 51 files; product imagery moves onto `ProductImage`, while QR codes, receipts and avatars keep plain `<img>` deliberately. Shrink-only guard added with the first batch. | pending, parallel-safe |
