...
| D10 | Standalone tinted text: one step, which one? The palette says the 400 step for elevated, danger, info, success and special, and amber-300 for warning. Badges all use 300 (text on a /15 wash), chips all use 400 (text on a /10 wash), so no existing rule explains why warning's bare-ground text alone sits at 300. Both steps clear 4.5:1 comfortably on both grounds (measured), so this is coherence, not legibility. Either warning drops to amber-400 so every tone's standalone text matches its neighbours, or the other five rise to 300 so bare-ground text is uniformly brighter than chip text. The first is a 38-site revisit of batch 3; the second is a larger, five-tone change. Not resolved autonomously because it reverses or extends a deliberate decision one batch old, which is exactly the churn the operating loop exists to stop. | 6.2 fold batches (orange fold waits on this so it folds in the right direction) |
