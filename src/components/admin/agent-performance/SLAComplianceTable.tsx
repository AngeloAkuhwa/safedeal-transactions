      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4 xl:grid-cols-6">
        {[
          ...
          <div key={label} title={hint ?? undefined} className={cn(INNER_CARD_CLASS, "p-3")}>
            ...
            {hint && <div className="mt-1 text-xs text-muted-foreground">n = {String(hint).match(/\d+/)?.[0] ?? 0}</div>}
          </div>
        ))}
      </div>
      <div className={cn(INNER_CARD_CLASS, "flex flex-wrap items-end justify-between gap-4")}>
        <div>
          <div className="text-xs uppercase tracking-wide text-muted-foreground">SLA compliance · {rangeLabel}</div>
                ...
                return (
                  <tr key={`${c.source ?? "task"}-${c.id}`} className="border-b border-border/60 transition-colors hover:bg-card/50">
                    <td className="px-3 py-2.5 text-sm">
                      ...
                      <div className="max-w-[220px] truncate text-xs text-muted-foreground">{c.title ?? c.type}</div>
                    </td>
                    ...
                    <td className="px-3 py-2.5">
                      <span className={cn("rounded-full px-2 py-0.5 text-xs", STATE_META[c.sla_state].className)}>
                        {STATE_META[c.sla_state].label}
                      </span>
                      {c.source === "dispute" && (
                        <span className="ml-1 rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">Dispute only</span>
                      )}
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-right">
                      <div className="flex justify-end gap-1.5">
                        ...
                        <button
                          ...
                          className="rounded-lg border border-border/70 bg-card/60 px-2 py-1 text-xs text-muted-foreground transition-colors hover:bg-muted hover:text-foreground min-h-11"
                        >
                          Escalate
                        </button>
                        )}
