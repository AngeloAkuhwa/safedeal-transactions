  ...
  return (
    <div className="hidden lg:block bg-slate-900 border border-slate-800 rounded-xl">
      <div className="p-6 border-b border-slate-800 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <h3 className="text-white text-lg font-semibold">Flagged Users List</h3>
        ...
        <table className="w-full sd-stack">
          <thead className="bg-slate-800 border-b border-slate-700">
            <tr>
              ...
              <th className="text-left p-4 text-slate-300 font-semibold text-sm whitespace-nowrap">Risk Level</th>
              <th className="text-left p-4 text-slate-300 font-semibold text-sm whitespace-nowrap">Related Context</th>
              <th className="text-left p-4 text-slate-300 font-semibold text-sm whitespace-nowrap">Flagged By</th>
              <th className="text-left p-4 text-slate-300 font-semibold text-sm whitespace-nowrap">Date</th>
              <th className="text-center p-4 text-slate-300 font-semibold text-sm whitespace-nowrap">Actions</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-800">
            {rows.length === 0 && (
              <tr><td colSpan={8} className="p-12 text-center text-slate-500 text-sm">No flagged users match the current filters.</td></tr>
            )}
            {rows.map((r) => {
              ...
              return (
                <tr key={r.user_id} className={`hover:bg-slate-800/50 transition-all border-l-4 ${RISK_BORDER[r.risk]}`}>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-slate-400 text-xs truncate max-w-[180px]">{r.email ?? "—"}</p>
                        <p className="text-slate-500 text-xs">ID: {r.short_id}</p>
                      </div>
                      <div className="flex items-center gap-1">
                        {r.risk === "critical" && <AlertTriangle className="h-3 w-3 text-red-400" />}
                    ...
                    <div className="space-y-2 min-w-[260px]">
                      {r.related.tx_code && (
                        <div className="flex items-start gap-2">
                          <div className="w-5 h-5 bg-purple-500/10 border border-purple-500/30 rounded flex items-center justify-center flex-shrink-0 mt-0.5">
                            <Banknote className="h-3 w-3 text-purple-400" />
                          </div>
                          <div>
                            <p className="text-white text-sm font-medium">#{r.related.tx_code}</p>
                            <p className="text-slate-400 text-xs">
                      ...
                      {r.disputes_30d > 0 && (
                        <div className="flex items-start gap-2">
                          <div className="w-5 h-5 bg-orange-500/10 border border-orange-500/30 rounded flex items-center justify-center flex-shrink-0 mt-0.5">
                      ...
                      {r.refunds_30d >= 2 && (
                        <div className="flex items-center gap-1.5 px-2 py-1 bg-red-500/10 border border-red-500/20 rounded">
                          ...
                          <p className="text-red-400 text-xs font-semibold">Pattern: Repeated chargebacks ({r.refunds_30d} in 30 days)</p>
                        </div>
                      )}
                      {r.auto_detected && (
                      ...
                      {r.flagged_by.is_system ? (
                        <>
                          ...
                          <div>
                            ...
                            <p className="text-slate-400 text-xs">Fraud Algorithm</p>
                          </div>
                        </>
                      ) : (
                        <>
                          ...
                          <div>
                            <p className="text-white text-sm font-medium">{r.flagged_by.name}</p>
                  ...
                  <td className="p-4">
                    <p className="text-slate-300 text-sm">{relative(r.flagged_at)}</p>
                    <p className="text-slate-500 text-xs">{absoluteDate(r.flagged_at)}</p>
                  </td>
                  <td className="p-4">
