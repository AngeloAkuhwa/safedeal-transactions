      ...
      <div className="p-4 flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2">
            ...
            <span className="text-[10px] text-slate-500 font-medium whitespace-nowrap">{relative(row.flagged_at)}</span>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mt-3">
            {row.reasons.slice(0, 2).map((r) => (
              <span key={r.key} className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider border ${REASON_PILL[r.key] ?? "bg-slate-800 border-slate-700 text-slate-300"}`}>
                {r.label}
              </span>
            ))}
          ...
          <div className="p-4 space-y-3 bg-slate-900/50">
            ...
            {row.auto_detected && (
              <div className="mt-2 p-3 bg-blue-500/5 border border-blue-500/20 rounded-xl flex items-center gap-2">
                ...
                <span className="text-[11px] text-blue-300">Auto-detected by fraud algorithm</span>
              </div>
            )}
          </div>
        </details>
