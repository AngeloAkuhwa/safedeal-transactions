...
interface Props {
  ...
  onConfirm: (p: { reason: string; target_money_status: "funds_held_in_escrow"|"funds_pending_release"; note?: string; acknowledge_open_dispute?: boolean }) => Promise<void> | void;
}
...
export function UnfreezeFundsDialog({ open, onOpenChange, bothPartiesConfirmed, hasActiveDispute, onConfirm }: Props) {
  ...
  const close = (v: boolean) => {
    ...
    if (!v) { setCategory(REASONS[0]); setOtherReason(""); setNote(""); setConfirmed(false); setAck(false); }
    onOpenChange(v);
  };
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      await onConfirm({ reason: reasonText, target_money_status: target, note: note.trim() || undefined, acknowledge_open_dispute: needsAck ? ack : undefined });
      close(false);
