          ...
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_22rem]">
            ...
            <div className="min-w-0 space-y-5">
              ...
              <section className="rounded-xl border border-border/60 bg-card/40 p-4 space-y-2">
                <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Current role</div>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {user.roles.filter((r) => r !== user.primary_role).map((r) => (
                    <Badge key={r} variant="outline" className="text-xs">{ROLE_LABEL[r]}</Badge>
                  ))}
                  <Badge variant="outline" className={cn("ml-auto text-xs", LEVEL_BADGE[user.access_level])}>
              ...
              {/* Dates */}
              <section className="rounded-xl border border-border/60 bg-card/40 p-4 space-y-3">
                <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Effective window</div>
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <div className="space-y-1.5">
                    ...
                    {expires && effective && expires <= effective && (
                      <div className="text-xs text-rose-400">Expiry must be after the effective date.</div>
                    )}
                  </div>
                </div>
                {expires && (
                  <div className="rounded-md border border-sky-500/30 bg-sky-500/10 px-2.5 py-1.5 text-xs text-sky-200">
              ...
              <section className="rounded-xl border border-border/60 bg-card/40 p-4 space-y-2">
                ...
                />
                <div className="text-xs text-muted-foreground">{reason.trim().length}/12 minimum</div>
              </section>
                  ...
                  <>
                    ...
                    <div className="space-y-1">
                      ...
                      {diff.unavailableModules.length === 0
                        ? <div className="pl-5 text-xs text-muted-foreground">None</div>
                        : (
                          <div className="flex flex-wrap gap-1.5 pl-5">
                            {diff.unavailableModules.map((m) => (
                              <span key={m.key} className="rounded-md border border-rose-500/30 bg-rose-500/10 px-2 py-0.5 text-xs text-rose-200">
                                {m.label}
                              </span>
                            ))}
                          </div>
...
function SectionCap({ children, icon, tone }: { children: React.ReactNode; icon: React.ReactNode; tone: "emerald" | "rose" | "amber" | "sky" }) {
  ...
  return (
    <div className={cn("flex items-center gap-1.5 text-xs font-semibold uppercase tracking-[0.14em]", toneClass)}>
      {icon}
      {children}
    </div>
  );
      ...
      {items.length === 0
        ...
        : (
          <div className="space-y-0.5 pl-5">
            {shown.map((k) => (
              <div key={k} className="font-mono text-xs text-foreground/80">{k}</div>
            ))}
