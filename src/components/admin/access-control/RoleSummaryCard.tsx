          ...
          {bucketStats.map((b) => {
            ...
            return (
              <div key={b.key} className="grid grid-cols-[6.25rem_1fr_auto] items-center gap-2">
                <div className={`flex items-center gap-1.5 text-[11px] ${muted ? "text-muted-foreground" : b.text}`}>
      ...
      <section className="space-y-1.5 border-t border-border/60 pt-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          ...
          {shownModules.map((m) => (
            <span
              key={m.key}
              className="rounded-md border border-border bg-muted/60 px-2 py-0.5 text-[11px] text-foreground/80 transition-colors hover:bg-muted"
            >
              {m.label}
            </span>
          ))}
      ...
      {restricted.length > 0 && (
        <section className="space-y-1.5 border-t border-border/60 pt-3">
          ...
          <div className="space-y-1">
            ...
            {extraRestricted > 0 && (
              <div className="pl-4 text-[11px] text-muted-foreground">+{extraRestricted} more restricted</div>
            )}
          </div>
        </section>
      )}
