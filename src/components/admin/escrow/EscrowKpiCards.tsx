...
function Card({ icon, iconBg, iconBorder, iconColor, label, value, sub, delta, deltaColor }: {
  icon: React.ReactNode; iconBg: string; iconBorder: string; iconColor: string;
  label: string; value: number; sub: string; delta: number; deltaColor: string;
}) {
  return (
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3 lg:gap-4">
      <Card
        ...
        label="Total Held in Escrow" value={kpis.total_held} sub={`Across ${kpis.total_held_count.toLocaleString("en-NG")} transactions`} delta={kpis.total_held_delta_pct} deltaColor="text-emerald-400"
      />
      <Card
        icon={<Lock className="h-5 w-5" />} iconBg="bg-red-500/10" iconBorder="border-red-500/30" iconColor="text-red-400"
        label="Total Frozen" value={kpis.total_frozen} sub={`${kpis.total_frozen_count.toLocaleString("en-NG")} frozen transactions`} delta={kpis.total_frozen_delta_pct} deltaColor="text-red-400"
      />
      <Card
        icon={<Hourglass className="h-5 w-5" />} iconBg="bg-orange-500/10" iconBorder="border-orange-500/30" iconColor="text-orange-400"
        label="Pending Release" value={kpis.pending_release} sub={`${kpis.pending_release_count.toLocaleString("en-NG")} pending releases`} delta={kpis.pending_release_delta_pct} deltaColor="text-orange-400"
      />
      <Card
        icon={<RotateCcw className="h-5 w-5" />} iconBg="bg-purple-500/10" iconBorder="border-purple-500/30" iconColor="text-purple-400"
        label="Total Refunded" value={kpis.total_refunded} sub={`${kpis.total_refunded_count.toLocaleString("en-NG")} refunded today`} delta={kpis.total_refunded_delta_pct} deltaColor="text-purple-400"
      />
      <Card
        icon={<CheckCircle2 className="h-5 w-5" />} iconBg="bg-emerald-500/10" iconBorder="border-emerald-500/30" iconColor="text-emerald-400"
        label="Released Today" value={kpis.released_today} sub={`${kpis.released_today_count.toLocaleString("en-NG")} transactions`} delta={kpis.released_today_delta_pct} deltaColor="text-emerald-400"
      />
      <Card
        icon={<CalendarRange className="h-5 w-5" />} iconBg="bg-cyan-500/10" iconBorder="border-cyan-500/30" iconColor="text-cyan-400"
        label="Released This Week" value={kpis.released_week} sub="7 days total" delta={kpis.released_week_delta_pct} deltaColor="text-cyan-400"
      />
    </div>
  );
}