            ...
            return (
              <tr key={`${r.permissionKey}-${r.op}`} className="border-t border-border/40 hover:bg-muted/20">
                <td className="px-3 py-2 font-mono text-[12px]">
                  ...
                  {entry?.label && <div className="text-[12px] text-muted-foreground">{entry.label}</div>}
                </td>
                <td className="px-3 py-2 text-muted-foreground">{entry?.moduleLabel ?? r.permissionKey.split(".")[0]}</td>
                <td className="px-3 py-2">
                  <span className={cn("rounded px-1.5 py-0.5 text-[12px] uppercase font-medium", riskTone)}>{risk}</span>
                </td>
                <td className="px-3 py-2">
                  <span className={cn("inline-flex items-center gap-1 rounded px-2 py-0.5 text-[12px] font-medium",
