  ...
  return (
    <section id={id} className="rounded-xl border border-border/60 bg-card/40">
      ...
      >
        ...
        {count !== undefined && (
          <span className="ml-auto rounded-full border border-border/60 bg-background/60 px-2 py-0.5 text-xs text-muted-foreground">
            {count}
          </span>
        )}
      </button>
          ...
          <ul className="grid gap-1.5 sm:grid-cols-2 lg:grid-cols-3">
            {summary.deniedKeys.map((k) => (
              <li key={k} className="flex items-center gap-2 rounded-md border border-border bg-background/30 px-2 py-1.5 text-xs text-muted-foreground">
                ...
                <span className="ml-auto font-mono text-xs">{k.split(".")[0]}</span>
              </li>
            ))}
          </ul>
        )}
          ...
          <ul className="divide-y divide-border/60 rounded-md border border-border/60">
            {usersQuery.data.map((u) => (
              <li key={u.id} className="flex items-center gap-3 px-3 py-2 text-sm">
                ...
                {u.is_primary && <span className="rounded-full border border-primary/30 bg-primary/10 px-2 py-0.5 text-xs text-primary">Primary</span>}
                {u.status && u.status !== "active" && (
                  <span className="rounded-full border border-border bg-muted/40 px-2 py-0.5 text-xs text-muted-foreground">{u.status}</span>
                )}
                <button
                  type="button"
                  onClick={() => navigate(`/admin/users/${u.id}/profile`)}
                  className="text-xs text-primary hover:underline min-h-11 inline-flex items-center px-3"
                >
                  Open
                </button>
              </li>
