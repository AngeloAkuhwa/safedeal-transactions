        ...
        {meta && (
          <div className="mt-4 space-y-5 text-sm">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span className="rounded-md border border-border bg-background px-2 py-0.5 font-mono text-xs text-muted-foreground">{meta.key}</span>
              <span className="rounded-full border border-border/60 bg-background/60 px-2 py-0.5 text-xs uppercase text-muted-foreground">
                {(meta.status ?? "active")}
              </span>
              {canManage && onEdit && (
                <button
                  ...
                  className="ml-auto inline-flex items-center gap-1 rounded-md border border-border bg-background px-2 py-1 text-xs hover:bg-muted min-h-11"
                >
              ...
              {deps.length === 0
                ...
                : (
                  <ul className="flex flex-wrap gap-1">
                    {deps.map((d) => (
                      <li key={d} className="rounded-md border border-border bg-background/60 px-2 py-0.5 font-mono text-xs text-muted-foreground">{d}</li>
                    ))}
                  </ul>
                )}
            </div>
              ...
              {overridesQuery.data && overridesQuery.data.length > 0 && (
                <ul className="space-y-1">
                  {overridesQuery.data.slice(0, 8).map((o) => (
                    <li key={`${o.user_id}-${o.created_at}`} className="rounded-md border border-border/60 bg-background/40 px-2 py-1 text-xs">
