      ...
      <SheetContent side="right" className="w-full overflow-y-auto sm:max-w-xl">
        ...
        <div className="mt-4 space-y-4 text-sm">
          ...
          <section className="space-y-2">
            <div className="text-[12px] uppercase text-muted-foreground">User</div>
            ...
            <div className="max-h-40 overflow-y-auto rounded-md border border-border/60">
              {users.map((u) => (
                <button type="button" key={u.id} onClick={() => setUser(u)}
                  ...
                  <span className="rounded-full border border-border bg-background px-2 py-0.5 text-[12px] text-muted-foreground">{u.role ? ROLE_LABEL[u.role] : "no role"}</span>
                </button>
              ))}
              {users.length === 0 && <div className="px-2 py-2 text-[12px] text-muted-foreground">No matches</div>}
            </div>
          </section>

          {/* Permission picker */}
          <section className="space-y-2">
            <div className="text-[12px] uppercase text-muted-foreground">Permission</div>
            <div className="flex items-center gap-2 rounded-md border border-border bg-background px-2">
              <Search className="h-3 w-3 text-muted-foreground" />
            ...
            <div className="max-h-40 overflow-y-auto rounded-md border border-border/60">
              {permOptions.map((p) => (
                <button type="button" key={p.key} onClick={() => setPermKey(p.key)}
                  ...
                  <span className="font-mono text-[12px] text-muted-foreground">{p.key}</span>
          ...
          {/* Type */}
          <section className="space-y-2">
            <div className="text-[12px] uppercase text-muted-foreground">Override type</div>
          ...
          <section className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <label className="block sm:col-span-2">
              <span className="mb-1 block text-[12px] uppercase text-muted-foreground">Reason (min 20 chars)</span>
            ...
            <label className="block sm:col-span-2">
              <span className="mb-1 flex items-center gap-1 text-[12px] uppercase text-muted-foreground">
              ...
              {privileged && type === "grant" && (
                <div className="mt-1 flex items-center gap-1 text-[12px] text-amber-300">
          ...
          <section className="space-y-2 rounded-md border border-border/60 bg-background/40 p-3">
            <div className="text-[12px] uppercase text-muted-foreground">Impact preview</div>
            ...
            {!guard.ok && (guard as any).message && (
              <div className={`flex items-start gap-2 rounded-md border p-2 text-[12px] ${(guard as any).blocked ? "border-rose-500/40 bg-rose-500/10 text-rose-200" : "border-amber-500/30 bg-amber-500/10 text-amber-200"}`}>
