...
export function PermissionFilters({
  ...
  return (
    <div className="flex flex-wrap items-end gap-3 rounded-xl border border-border/70 bg-card/60 p-3 backdrop-blur-sm shadow-[0_1px_0_hsl(var(--border)/0.4)_inset]">
      ...
      {showModule && (
        <label>
          <span className="mb-1 block text-xs font-medium uppercase tracking-wide text-muted-foreground">Feature Group</span>
          <select
            className="h-11 rounded-md border border-border bg-background/60 px-3 text-sm outline-none focus:border-primary"
            value={value.module}
      ...
      {showAction && (
        <label>
          <span className="mb-1 block text-xs font-medium uppercase tracking-wide text-muted-foreground">Action</span>
          <select
            className="h-11 rounded-md border border-border bg-background/60 px-3 text-sm outline-none focus:border-primary"
      ...
      {showRisk && (
        <label>
          <span className="mb-1 block text-xs font-medium uppercase tracking-wide text-muted-foreground">Risk</span>
          <select
            className="h-11 rounded-md border border-border bg-background/60 px-3 text-sm outline-none focus:border-primary"
            value={value.risk}
      ...
      {showStatus && (
        <label>
          <span className="mb-1 block text-xs font-medium uppercase tracking-wide text-muted-foreground">Status</span>
          <select
            className="h-11 rounded-md border border-border bg-background/60 px-3 text-sm outline-none focus:border-primary"
      ...
      {showApproval && (
        <label>
          <span className="mb-1 block text-xs font-medium uppercase tracking-wide text-muted-foreground">Approval</span>
          <select
            className="h-11 rounded-md border border-border bg-background/60 px-3 text-sm outline-none focus:border-primary"
      ...
      <div className="ml-auto flex flex-wrap items-center gap-2 text-xs text-muted-foreground">
        <span className={`inline-flex items-center gap-1 rounded-full border ${ADMIN_TONE.success.panel} px-2 py-0.5 text-emerald-300`}><span className="h-1.5 w-1.5 rounded-full bg-emerald-400" /> Full</span>
        <span className={`inline-flex items-center gap-1 rounded-full border ${ADMIN_TONE.warning.panel} px-2 py-0.5 text-amber-200`}><span className="h-1.5 w-1.5 rounded-full bg-amber-400" /> Limited</span>
