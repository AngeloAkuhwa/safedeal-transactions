      ...
      <SheetContent side="right" className="w-full max-w-md overflow-y-auto sm:max-w-2xl">
        <SheetHeader>
          <SheetTitle className="flex items-center gap-2">
            Review changes
            <span className={`ml-2 rounded px-2 py-0.5 text-[10px] uppercase ${changeStateTone("draft").bg} ${changeStateTone("draft").text}`}>Draft</span>
          </SheetTitle>
        ...
        <div className="mt-4 space-y-4">
            ...
            return (
              <div key={role} className="rounded-xl border border-border/60 bg-background/40 p-3">
                <div className="mb-2 flex flex-wrap items-center gap-2">
                  ...
                  <span className="rounded bg-emerald-500/15 px-1.5 py-0.5 text-[10px] font-medium text-emerald-300">+{adds.length}</span>
                  <span className="rounded bg-rose-500/15 px-1.5 py-0.5 text-[10px] font-medium text-rose-300">−{removes.length}</span>
                  {(userImpactByRole?.get(role) ?? 0) > 0 && (
                    <span className="ml-auto inline-flex items-center gap-1 rounded bg-muted px-2 py-0.5 text-[10px] text-muted-foreground">
                ...
                {requires ? (
                  ...
                  <div className="mt-2 flex items-center gap-2 rounded border border-emerald-500/30 bg-emerald-500/5 p-2 text-[11px] text-emerald-200">
          ...
          <div className="rounded-xl border border-border/60 bg-background/40 p-3">
            ...
            <Textarea
              ...
              className="text-sm"
            />
            <div className={`mt-1 text-[11px] ${reasonOk ? "text-emerald-300" : "text-muted-foreground"}`}>
              {reason.trim().length}/20 characters
            </div>
          </div>

          {anyRequires && (
            <div className="flex items-start gap-2 rounded border border-sky-500/30 bg-sky-500/5 p-2 text-[11px] text-sky-200">
