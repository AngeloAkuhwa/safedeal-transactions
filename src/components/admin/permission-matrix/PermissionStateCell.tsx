...
export function PermissionStateCell({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        "inline-flex h-11 min-w-[110px] items-center justify-center gap-1.5 whitespace-nowrap rounded-full px-3 text-xs font-semibold tracking-wide transition",
        STYLES[state],
    ...
    >
      ...
      {state === "partial" && total != null && (
        <span
          className={cn(
            "ml-1 rounded-md px-1.5 py-0.5 font-mono text-xs leading-none",
            BADGE_STYLES[state],
          )}
        >
          {granted}/{total}
