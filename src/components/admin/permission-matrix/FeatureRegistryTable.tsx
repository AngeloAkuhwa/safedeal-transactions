...
export function FeatureRegistryTable({
  ...
  const rows = useMemo(() => {
    ...
    const out: Array<{
      ...
      privileged: boolean; grantedRoles: string[]; overrideCount: number;
    }> = [];
    for (const m of PERMISSION_MODULES) {
      ...
      for (const p of m.permissions) {
        ...
        out.push({
          ...
          privileged: priv,
          grantedRoles,
          overrideCount: overrideCountByPerm.get(p.key) ?? 0,
        });
      }
    }
    return out;
          ...
          <tbody>
            {rows.map((r) => (
              ...
              >
                ...
                <td className="px-4 py-2"><PermissionRiskBadge privileged={r.privileged} /></td>
