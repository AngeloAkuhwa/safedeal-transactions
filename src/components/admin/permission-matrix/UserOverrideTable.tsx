  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border/60 bg-card/40 p-3">
        <div className="flex items-center gap-1 text-[12px] uppercase text-muted-foreground"><Filter className="h-3 w-3" /> Filter</div>
        ...
        {(filters.q || filters.type !== "all" || filters.status !== "all" || filters.role !== "all" || filters.module !== "all") && (
          <button type="button" onClick={() => setFilters(DEFAULT_FILTERS)} className="text-[12px] text-muted-foreground hover:text-foreground min-h-11 inline-flex items-center">Clear</button>
        )}
        {canEdit && onCreate && (
            ...
            {filtered.map((r, i) => {
              ...
              >
                <td className="px-4 py-3 align-middle">
                  ...
                  <div className="text-[12px] text-muted-foreground">{r.user_email}</div>
                </td>
                ...
                <td className="px-4 py-3 align-middle">
                  <div className="flex items-center gap-2">
                    <div>
                      ...
                      <div className="text-[12px] font-mono text-muted-foreground">{r.permission_key}</div>
                    </div>
                    {r.privileged && <PermissionRiskBadge privileged size="xs" />}
                  </div>
                </td>
                <td className="px-4 py-3 align-middle">
                  <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[12px] font-semibold ${t.cls}`}>{t.label}</span>
                </td>
                <td className="px-4 py-3 align-middle">
                ...
                <td className="px-4 py-3 align-middle text-[12px] text-muted-foreground">{new Date(r.created_at).toLocaleDateString()}</td>
                <td className="px-4 py-3 align-middle text-[12px]">
                  <span className={
                ...
                <td className="px-4 py-3 align-middle">
                  <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-[12px] ${s.cls}`}>{s.label}</span>
                </td>
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-2 text-sm">
          ...
          {!guard.ok && guard.message && (
            <div className="rounded-md border border-amber-500/30 bg-amber-500/10 p-2 text-[12px] text-amber-200">{guard.message}</div>
          )}
          <label className="block">
            <span className="mb-1 block text-[12px] uppercase text-muted-foreground">Reason (required)</span>
