  ...
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center gap-2 rounded-xl border border-border/60 bg-card/40 p-3">
        <div className="flex items-center gap-1 text-xs uppercase text-muted-foreground"><Filter className="h-3 w-3" /> Filter</div>
        ...
        {(filters.q || filters.type !== "all" || filters.status !== "all" || filters.role !== "all" || filters.module !== "all") && (
          <button type="button" onClick={() => setFilters(DEFAULT_FILTERS)} className="text-xs text-muted-foreground hover:text-foreground min-h-11 inline-flex items-center px-3">Clear</button>
        )}
        {canEdit && onCreate && (
          ...
          <tbody>
            {filtered.map((r, i) => {
              ...
              >
                ...
                <td className="px-4 py-3 align-middle">
                  <div className="flex items-center gap-2">
                    <div>
                      ...
                      <div className="text-xs font-mono text-muted-foreground">{r.permission_key}</div>
                    </div>
                    {r.privileged && <PermissionRiskBadge privileged size="xs" />}
                  </div>
                </td>
                <td className="px-4 py-3 align-middle">
                  <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs font-semibold ${t.cls}`}>{t.label}</span>
                </td>
                <td className="px-4 py-3 align-middle">
                ...
                <td className="px-4 py-3 align-middle text-xs text-muted-foreground">{new Date(r.created_at).toLocaleDateString()}</td>
                ...
                <td className="px-4 py-3 align-middle">
                  <span className={`inline-flex items-center rounded-full border px-2 py-0.5 text-xs ${s.cls}`}>{s.label}</span>
                </td>
                ...
                <td className="px-4 py-3 align-middle text-right">
                  <div role="button" tabIndex={0} onKeyDown={keyActivate} className="flex justify-end gap-1" onClick={(e) => e.stopPropagation()}>
                    <button type="button" onClick={() => onRowClick?.(r)}
                      className="inline-flex items-center gap-1 rounded-md border border-border bg-background px-2 py-1 text-xs hover:bg-muted min-h-11">
                    ...
                    {canEdit && (
                      <>
                        ...
                        <button type="button" onClick={() => setRevokeFor(r)}
                          className="inline-flex items-center gap-1 rounded-md border border-rose-500/30 bg-rose-500/10 px-2 py-1 text-xs text-rose-300 hover:bg-rose-500/20 min-h-11">
                    ...
                    <Link
                      ...
                      className="inline-flex items-center gap-1 rounded-md border border-border px-2 py-1 text-xs hover:bg-muted min-h-11"
                    >
      ...
      <DialogContent className="max-w-sm">
        ...
        <div className="space-y-2 text-sm">
          ...
          <label className="block">
            <span className="mb-1 block text-xs uppercase text-muted-foreground">Extend by (days)</span>
