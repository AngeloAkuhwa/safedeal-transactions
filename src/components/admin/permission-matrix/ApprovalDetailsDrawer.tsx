        ...
        <div className="mt-4 space-y-4 text-sm">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-md border border-border/60 bg-background/40 p-3">
              <div className="mb-1 text-xs uppercase text-muted-foreground">Impacted role</div>
              <div className="text-xs">{impactedRoleLabel ?? <span className="italic text-muted-foreground">n/a</span>}</div>
            </div>
            <div className="rounded-md border border-border/60 bg-background/40 p-3">
              <div className="mb-1 text-xs uppercase text-muted-foreground">Impacted users (est.)</div>
              ...
              <ul className="space-y-1 text-xs">
                {projected.conflicts.map((c, i) => (
                  <li key={i} className="rounded border border-rose-500/40 bg-rose-500/10 p-2 text-rose-200">
                    ...
                    <span className="ml-2 uppercase text-xs">{c.severity}</span>
                    {c.rationale && <div className="mt-1 text-xs opacity-80">{c.rationale}</div>}
                  </li>
                ))}
              </ul>
            )}
          ...
          {!isTerminal && (
            <div className="space-y-2 rounded-lg border border-border/60 bg-background/40 p-3">
              ...
              {!guard.allowed && (
                <div className="flex items-start gap-2 rounded-md bg-amber-500/10 p-2 text-xs text-amber-300">
  ...
  return (
    <li className="flex items-start gap-2 rounded border border-border/40 bg-background/40 p-2">
      ...
      <div className="flex-1">
        <div className="text-xs font-medium capitalize">{label}{who ? <span className="ml-1 text-muted-foreground">· {who}</span> : null}</div>
        {when && <div className="text-xs text-muted-foreground">{format(new Date(when), "PPpp")}</div>}
        {comment && <div className="mt-1 text-xs text-muted-foreground">{comment}</div>}
      </div>
    </li>
  );
}