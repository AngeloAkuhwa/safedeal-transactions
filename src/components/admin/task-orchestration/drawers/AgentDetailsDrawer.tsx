      ...
      <SheetContent side="right" className="w-full sm:max-w-md overflow-y-auto bg-[radial-gradient(ellipse_at_top_right,hsl(var(--primary)/0.05),transparent_55%)]">
        ...
        {agent && (
          <div className="mt-5 space-y-4">
            ...
            <div className="rounded-xl border border-border/60 bg-card/40 p-3 backdrop-blur-sm shadow-[0_1px_0_hsl(var(--border)/0.4)_inset]">
              <div className="mb-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">Role &amp; Skills</div>
              <div className="flex flex-wrap gap-1.5">
                {(agent.role || agent.role_key) && (
                  <span className="inline-flex items-center gap-1 rounded-md bg-primary/10 px-2 py-0.5 text-[11px] font-medium text-primary ring-1 ring-inset ring-primary/25">
                    <Users className="h-3 w-3" /> {agent.role ?? humanize(agent.role_key!)}
                  </span>
                )}
                {(agent.skills ?? []).length === 0 && !agent.role && !agent.role_key && (
                  <span className="text-[11px] text-muted-foreground">No skills tagged.</span>
                )}
                {(agent.skills ?? []).map(s => (
                  <span key={s} className="rounded-md bg-muted/60 px-2 py-0.5 text-[11px] text-muted-foreground ring-1 ring-inset ring-border">
                    {humanize(s)}
                  </span>
                ))}
              </div>
            ...
            <div className="rounded-xl border border-border/60 bg-card/40 p-3 backdrop-blur-sm shadow-[0_1px_0_hsl(var(--border)/0.4)_inset] text-[11px] text-muted-foreground">
              {agent.email && (
