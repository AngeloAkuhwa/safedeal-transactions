              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="mb-1 flex flex-wrap items-center gap-2">
                    ...
                    <span className={cn("inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold", priorityBadgeClass(m.priority as any))}>
                      {humanize(m.priority)}
                    </span>
                    {m.sla_delta && (
                      <span className={cn("inline-flex rounded-full px-2 py-0.5 text-[10px] font-semibold", slaBadgeClass(m.sla_delta))}>
                        SLA {slaLabel(m.sla_delta)}
                      </span>
                    )}
                    <span className="text-[10px] text-muted-foreground">· {humanize(m.reason)}</span>
                  </div>
                  <div className="flex items-center gap-2 text-[11px] text-muted-foreground">
                    ...
                    {to && (
                      <span className="ml-1 text-[10px]">({to.active} → {to.active + 1} / {to.max_active})</span>
                    )}
                  </div>
                </div>
              </label>
        ...
        {skipped.length > 0 && (
          <>
            <div className="mt-5 mb-2 flex items-center gap-2 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              ...
              <span className="rounded-full bg-muted/60 px-1.5 py-0.5 text-[10px] tabular-nums">{skipped.length}</span>
            </div>
