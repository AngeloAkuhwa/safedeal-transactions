            ...
            {tasks.map(t => {
              ...
              return (
                <tr key={t.id} className="border-b border-border/40 transition-colors hover:bg-muted/30">
                  ...
                  <td className="px-2 py-3">
                    <span className={cn("inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold", priorityBadgeClass(t.priority))}>
                      {humanize(t.priority)}
                    </span>
                  </td>
                  ...
                  <td className="px-2 py-3">
                    ...
                    {t.sla_due_at && <div className="mt-0.5 text-xs text-muted-foreground">Due {relativeShort(t.sla_due_at)}</div>}
                  </td>
                  ...
                  <td className="px-2 py-3 text-xs text-muted-foreground">{t.queue ? humanize(t.queue) : "Default"}</td>
                  <td className="px-2 py-3 text-xs text-muted-foreground">{suggested ? shortNameOf(suggested) : "Auto"}</td>
            ...
            <div key={t.id} className="rounded-xl border border-border/60 bg-background/40 p-4">
              <div className="mb-2 flex items-start justify-between gap-3">
                <button onClick={() => onOpenDetail(t)} className="text-left min-h-11 inline-flex items-center">
                  ...
                  {t.dispute_id && <div className="text-xs text-muted-foreground">Dispute D-{t.dispute_id.slice(0, 8)}</div>}
                </button>
                <Checkbox checked={selectedIds.has(t.id)} onCheckedChange={() => onToggle(t.id)} />
              </div>
