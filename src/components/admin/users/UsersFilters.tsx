...
const VERIFS: Array<{ v: string; l: string }> = [
  ...
  { v: "phone", l: "Phone Verified" }, { v: "email", l: "Email Only" }, { v: "none", l: "Unverified" },
];
...
function Select({ value, options, onChange }: { value: string; options: { v: string; l: string }[]; onChange: (v: string) => void }) {
  return (
    <select
      ...
      className="bg-slate-800 border border-slate-700 text-slate-200 text-sm rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500"
    >
    ...
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 sm:p-6">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-3">
        <div className="md:col-span-2 relative">
          ...
          <input
            ...
            className="w-full bg-slate-800 border border-slate-700 text-slate-200 text-sm rounded-lg pl-9 pr-3 py-2 focus:outline-none focus:ring-2 focus:ring-emerald-500"
          />
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mt-4">
        ...
        <div className="flex items-center gap-2">
          <button type="button" onClick={onReset} className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm rounded-lg flex items-center gap-1.5">
            <X className="h-4 w-4" /> Reset
          </button>
          <button type="button" onClick={onApply} className="px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-medium rounded-lg flex items-center gap-1.5">
