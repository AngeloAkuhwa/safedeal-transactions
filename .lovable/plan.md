...
Also proved at H on real data: at most one `payout_debit` per payout and one `refund_debit` per refund, no open commitment behind a terminal operation row, and `committed_amount` never presented as both remaining and settled in any screen or export.
...
- Functions rewritten: **10** (#1–#10), with signature extensions on **6** of them (#2, #5, #6, #7, #9, #10); #1, #3, #4, #8 keep their current signatures and change body only. Unchanged but state-tested: **1** (#11). Functions added: **7** (`record_payment_capture_atomic`, `record_completion_release_intent_atomic`, `canonical_fingerprint_v1`, `escrow_committed_amount(uuid)` and `escrow_uncommitted_available(uuid)` implementing §3.1 — added as one migration-D pair, `claim_reconciliation_lease`, `heartbeat_reconciliation_lease`, `close_reconciliation_lease`) — total **8** new functions counting both lease helpers and both balance helpers. Ledger-writing RPCs after the change: **9** of the 10 rewritten plus the 2 new writers; the only `payout_debit` writer is #2 and the only `refund_debit` writer is #7.
