...
- Functions rewritten: **10** (#1–#10), signature extensions on **6** of them (#2, #5, #6, #7, #9, #10); #1, #3, #4 and #8 keep their current signatures and change body only. Unchanged but state-tested: **1** (#11). Functions added: **8** — `record_payment_capture_atomic`, `record_completion_release_intent_atomic`, `canonical_fingerprint_v1`, `escrow_committed_amount(uuid)`, `escrow_uncommitted_available(uuid)` (the §3.1 pair), `claim_reconciliation_lease`, `heartbeat_reconciliation_lease`, `close_reconciliation_lease`. `escrow_available_balance` is left semantically unchanged (cash only). After the change the only `payout_debit` writer is #2 and the only `refund_debit` writer is #7.
